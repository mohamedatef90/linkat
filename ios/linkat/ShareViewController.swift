...
//  linkat
//
//  Created by Mohamed Atef on 27/11/2025.
//
...
class ShareViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        // Title label
        titleLabel.text = "Save to Linkat"
    ...
    private func saveLinkToSharedStorage(url: String, name: String) {
        guard let userDefaults = UserDefaults(suiteName: appGroupId) else {
            print("linkat ShareExtension Error: Could not access UserDefaults with app group: \(appGroupId)")
            return
        }
        ...
        if let jsonData = try? JSONSerialization.data(withJSONObject: pendingLinks),
           let jsonString = String(data: jsonData, encoding: .utf8) {
            ...
            print("linkat ShareExtension: Saved link to UserDefaults - \(jsonString)")
        } else {
            print("linkat ShareExtension Error: Failed to encode JSON")
        }
    }
